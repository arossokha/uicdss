...
(function () {
    ...
    function b() {
        var C = this;
        var y = C._.currentTabId, z = C._.tabIdList.length, A = CKEDITOR.tools.indexOf(C._.tabIdList, y) + z;
        for (var B = A - 1; B > A - z; B--)if (a.call(C, C._.tabIdList[B % z]))return C._.tabIdList[B % z];
        return null;
    }
    ...
    function c() {
        var C = this;
        var y = C._.currentTabId, z = C._.tabIdList.length, A = CKEDITOR.tools.indexOf(C._.tabIdList, y);
        for (var B = A + 1; B < A + z; B++)if (a.call(C, C._.tabIdList[B % z]))return C._.tabIdList[B % z];
        return null;
    }
    ...
    CKEDITOR.dialog = function (y, z) {
        ...
        A = new j(this, A);
        this.definition = A = CKEDITOR.fire('dialogDefinition', {name:z, definition:A}, y).definition;
        var B = CKEDITOR.document, C = y.theme.buildDialog(y);
        this._ = {editor:y, element:C.element, name:z, contentSize:{width:0, height:0}, size:{width:0, height:0}, updateSize:false, contents:{}, buttons:{}, accessKeyMap:{}, tabs:{}, tabIdList:[], currentTabId:null, currentTabIndex:null, pageCount:0, lastTab:null, tabBarMode:false, focusList:[], currentFocusIndex:0, hasFocus:false};
        this.parts = C.parts;
        this.parts.dialog.setStyles({position:CKEDITOR.env.ie6Compat ? 'absolute' : 'fixed', top:0, left:0, visibility:'hidden'});
        CKEDITOR.event.call(this);
        if (A.onLoad)this.on('load', A.onLoad);
        ...
        var D = this, E = function (M) {
            var N = D._.contents, O = false;
        ...
        this.on('ok', function (M) {
            E(function (N) {
                if (N.validate) {
                    ...
                    if (O === false) {
                        if (N.select)N.select(); else N.focus();
                        M.data.hide = false;
                        M.stop();
                        return true;
                    }
        ...
        function G(M) {
            ...
            var N = M.data.getKeystroke(), O = false;
        ...
        this.on('show', function () {
            ...
            if (!P._.hasFocus) {
                ...
                if (P._.editor.mode == 'wysiwyg' && CKEDITOR.env.ie) {
                    var M = y.document.$.selection, N = M.createRange();
        ...
        if (CKEDITOR.env.ie6Compat)this.on('load', function (M) {
            var N = this.getElement(), O = N.getFirst();
            O.remove();
            O.appendTo(N);
        }, this);
        l(this);
        ...
        var I = /cke_dialog_tab(\s|$|_)/, J = /cke_dialog_tab(\s|$)/;
        this.parts.tabs.on('click', function (M) {
            var R = this;
            var N = M.data.getTarget(), O = N, P, Q;
        ...
        var K = [], L = CKEDITOR.dialog._.uiElementBuilders.hbox.build(this, {type:'hbox', className:'cke_dialog_footer_buttons', widths:[], children:A.buttons}, K).getChild();
    ...
    function e(y, z, A) {
        ...
        z.on('keydown', function (B) {
            if (B.data.getKeystroke() in {32:1, 13:1})this.fire('click');
        });
        z.on('focus', function () {
            this.fire('mouseover');
        });
    ...
    CKEDITOR.dialog.prototype = {resize:(function () {
        return function (y, z) {
            ...
            CKEDITOR.dialog.fire('resize', {dialog:A, skin:A._.editor.skinName, width:y, height:z}, A._.editor);
            A._.contentSize = {width:y, height:z};
            A._.updateSize = true;
        };
    })(), getSize:function () {
        var A = this;
        if (!A._.updateSize)return A._.size;
        var y = A._.element.getFirst(), z = A._.size = {width:y.$.offsetWidth || 0, height:y.$.offsetHeight || 0};
        ...
        return function (z, A) {
            ...
            D._.position = {x:z, y:A};
            if (!y) {
                var C = CKEDITOR.document.getWindow().getScrollPosition();
                z += C.x;
                A += C.y;
            }
            B.setStyles({left:(z > 0 ? z : 0) + ('px'), top:(A > 0 ? A : 0) + ('px')});
        };
    })(), getPosition:function () {
        return CKEDITOR.tools.extend({}, this._.position);
    }, show:function () {
        if (this._.editor.mode == 'wysiwyg' && CKEDITOR.env.ie)this._.editor.getSelection().lock();
        var y = this._.element, z = this.definition;
        if (!(y.getParent() && y.getParent().equals(CKEDITOR.document.getBody())))y.appendTo(CKEDITOR.document.getBody()); else return;
        ...
        CKEDITOR.tools.setTimeout(function () {
            var C = CKEDITOR.document.getWindow().getViewPaneSize(), D = this.getSize();
        ...
        var B = this;
        for (var z in B._.contents)for (var A in B._.contents[z])y(B._.contents[z][A]);
        return B;
    }, reset:(function () {
        var y = function (z) {
            if (z.reset)z.reset();
        ...
        for (var z in D._.tabs) {
            var A = D._.tabs[z][0], B = D._.tabs[z][1];
            if (z != y) {
    ...
    CKEDITOR.tools.extend(CKEDITOR.dialog, {add:function (y, z) {
        ...
        var y = function (z, A) {
            A = A || {};
            return CKEDITOR.tools.extend({id:'ok', type:'button', label:z.lang.common.ok, 'class':'cke_dialog_ui_button_ok', onClick:function (B) {
                var C = B.data.dialog;
                if (C.fire('ok', {hide:true}).hide !== false)C.hide();
            }}, A, true);
        };
        y.type = 'button';
        ...
        var y = function (z, A) {
            A = A || {};
            return CKEDITOR.tools.extend({id:'cancel', type:'button', label:z.lang.common.cancel, 'class':'cke_dialog_ui_button_cancel', onClick:function (B) {
                var C = B.data.dialog;
                if (C.fire('cancel', {hide:true}).hide !== false)C.hide();
            }}, A, true);
        };
        y.type = 'button';
        y.override = function (z) {
    ...
    var f = {resizable:CKEDITOR.DIALOG_RESIZE_NONE, minWidth:600, minHeight:400, buttons:[CKEDITOR.dialog.okButton, CKEDITOR.dialog.cancelButton]}, g = function (y, z, A) {
        ...
        this.dialog = y;
        var A = z.contents;
        for (var B = 0, C; C = A[B]; B++)A[B] = new k(y, C);
        CKEDITOR.tools.extend(this, z);
    };
    j.prototype = {getContents:function (y) {
    ...
    function l(y) {
        var z = null, A = null, B = y.getElement().getFirst(), C = y.getParentEditor(), D = C.config.dialog_magnetDistance, E = d[C.skinName].margins || [0, 0, 0, 0];

        function F(H) {
            var I = y.getSize(), J = CKEDITOR.document.getWindow().getViewPaneSize(), K = H.data.$.screenX, L = H.data.$.screenY, M = K - z.x, N = L - z.y, O, P;
            z = {x:K, y:L};
            A.x += M;
            A.y += N;
            if (A.x + E[3] < D)O = -E[3]; else if (A.x - E[1] > J.width - I.width - D)O = J.width - I.width + E[1]; else O = A.x;
            if (A.y + E[0] < D)P = -E[0]; else if (A.y - E[2] > J.height - I.height - D)P = J.height - I.height + E[2]; else P = A.y;
            y.move(O, P);
        ...
        y.parts.title.on('mousedown', function (H) {
            y._.updateSize = true;
            z = {x:H.data.$.screenX, y:H.data.$.screenY};
    ...
    ;
    function m(y) {
        var z = y.definition, A = z.minWidth || 0, B = z.minHeight || 0, C = z.resizable, D = d[y.getParentEditor().skinName].margins || [0, 0, 0, 0];
        ...
        ;
        var I = null, J = null, K = y._.editor.config.magnetDistance, L = ['tl', 't', 'tr', 'l', 'r', 'bl', 'b', 'br'];

        function M(P) {
            var Q = P.listenerData.part, R = y.getSize();
            J = y.getPosition();
            CKEDITOR.tools.extend(J, {x2:J.x + R.width, y2:J.y + R.height});
            I = {x:P.data.$.screenX, y:P.data.$.screenY};
            CKEDITOR.document.on('mousemove', N, y, {part:Q});
            CKEDITOR.document.on('mouseup', O, y, {part:Q});
            if (CKEDITOR.env.ie6Compat) {
                var S = o.getChild(0).getFrameDocument();
                S.on('mousemove', N, y, {part:Q});
                S.on('mouseup', O, y, {part:Q});
            }
        ...
        ;
        function N(P) {
            var Q = P.data.$.screenX, R = P.data.$.screenY, S = Q - I.x, T = R - I.y, U = CKEDITOR.document.getWindow().getViewPaneSize(), V = P.listenerData.part;
            ...
            I = {x:Q, y:R};
            var W, X, Y, Z;
            if (J.x + D[3] < K)W = -D[3]; else if (V.search('l') != -1 && J.x2 - J.x < A + K)W = J.x2 - A; else W = J.x;
            if (J.y + D[0] < K)X = -D[0]; else if (V.search('t') != -1 && J.y2 - J.y < B + K)X = J.y2 - B; else X = J.y;
            if (J.x2 - D[1] > U.width - K)Y = U.width + D[1]; else if (V.search('r') != -1 && J.x2 - J.x < A + K)Y = J.x + A; else Y = J.x2;
            if (J.y2 - D[2] > U.height - K)Z = U.height + D[2]; else if (V.search('b') != -1 && J.y2 - J.y < B + K)Z = J.y + B; else Z = J.y2;
            y.move(W, X);
            y.resize(Y - W, Z - X);
            P.data.preventDefault();
        }
    ...
    var n, o, p = function (y) {
        ...
        var C = o, D = function () {
            ...
            C.setStyles({width:G.width + 'px', height:G.height + 'px'});
        }, E = function () {
            var G = z.getScrollPosition(), H = CKEDITOR.dialog._.currentTop;
            C.setStyles({left:G.x + 'px', top:G.y + 'px'});
            do {
                var I = H.getPosition();
                H.move(I.x, I.y);
        ...
        var z = y.data.$.ctrlKey || y.data.$.metaKey, A = y.data.$.altKey, B = y.data.$.shiftKey, C = String.fromCharCode(y.data.$.keyCode), D = r[(z ? 'CTRL+' : '') + (A ? 'ALT+' : '') + (B ? 'SHIFT+' : '') + C];
        ...
        var D = r[A] || (r[A] = []);
        D.push({uiElement:y, dialog:z, key:A, keyup:C || y.accessKeyUp, keydown:B || y.accessKeyDown});
    }, v = function (y) {
        for (var z in r) {
            var A = r[z];
    ...
    (function () {
        CKEDITOR.ui.dialog = {uiElement:function (y, z, A, B, C, D, E) {
            if (arguments.length < 4)return;
            var F = (B.call ? B(z) : B) || ('div'), G = ['<', F, ' '], H = (C && C.call ? C(z) : C) || ({}), I = (D && D.call ? D(z) : D) || ({}), J = (E && E.call ? E(y, z) : E) || (''), K = this.domId = I.id || CKEDITOR.tools.getNextNumber() + '_uiElement', L = this.id = z.id, M;
            I.id = K;
            var N = {};
            ...
            if (arguments.length < 4)return;
            this._ || (this._ = {});
            var D = this._.children = z, E = C && C.widths || null, F = C && C.height || null, G = {}, H, I = function () {
                var J = ['<tbody><tr class="cke_dialog_ui_hbox">'];
                for (H = 0; H < A.length; H++) {
                    var K = 'cke_dialog_ui_hbox_child', L = [];
            ...
            CKEDITOR.ui.dialog.uiElement.call(this, y, C || {type:'hbox'}, B, 'table', G, C && C.align && {align:C.align} || null, I);
        }, vbox:function (y, z, A, B, C) {
            if (arguments.length < 3)return;
            this._ || (this._ = {});
            var D = this._.children = z, E = C && C.width || null, F = C && C.heights || null, G = function () {
                ...
                for (var I = 0; I < A.length; I++) {
                    ...
                    if (F)J.push('height:' + CKEDITOR.tools.cssLength(F[I])); else if (C && C.expand)J.push('height:' + Math.floor(100 / A.length) + '%');
            ...
            CKEDITOR.ui.dialog.uiElement.call(this, y, C || {type:'vbox'}, B, 'div', null, null, G);
        }};
    })();
    CKEDITOR.ui.dialog.uiElement.prototype = {getElement:function () {
        ...
        var z = /^on([A-Z]\w+)/, A, B = function (D, E, F, G) {
            E.on('load', function () {
        ...
        for (var C in y) {
            ...
            if (this.eventProcessors[C])this.eventProcessors[C].call(this, this._.dialog, y[C]); else B(this, this._.dialog, A[1].toLowerCase(), y[C]);
        }
        return this;
    }, eventProcessors:{onLoad:function (y, z) {
        y.on('load', z, this);
    }, onShow:function (y, z) {
        y.on('show', z, this);
    }, onHide:function (y, z) {
    ...
    CKEDITOR.ui.dialog.hbox.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.uiElement(), {getChild:function (y) {
        var z = this;
        if (arguments.length < 1)return z._.children.concat();
        if (!y.splice)y = [y];
        if (y.length < 2)return z._.children[y[0]]; else return z._.children[y[0]] && z._.children[y[0]].getChild ? z._.children[y[0]].getChild(y.slice(1, y.length)) : null;
    }}, true);
    CKEDITOR.ui.dialog.vbox.prototype = new CKEDITOR.ui.dialog.hbox();
    (function () {
        var y = {build:function (z, A, B) {
            var C = A.children, D, E = [], F = [];
    ...
    CKEDITOR.dialogCommand.prototype = {exec:function (y) {
    ...
    (function () {
        ...
        CKEDITOR.dialog.validate = {functions:function () {
            return function () {
                var H = this;
                var B = H && H.getValue ? H.getValue() : arguments[0], C = undefined, D = CKEDITOR.VALIDATE_AND, E = [], F;
                for (F = 0; F < arguments.length; F++)if (typeof arguments[F] == 'function')E.push(arguments[F]); else break;
                ...
                for (F = 0; F < E.length; F++)if (D == CKEDITOR.VALIDATE_AND)G = G && E[F](B); else G = G || E[F](B);
                if (!G) {
            ...
            return function () {
                ...
                if (!B.test(D)) {
                    if (C !== undefined)alert(C);
                    if (E && (E.select || E.focus))if (E.select)E.select(); else E.focus();
                    return false;
                }
                return true;
            };
